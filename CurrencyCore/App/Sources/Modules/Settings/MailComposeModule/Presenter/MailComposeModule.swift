...
final class MailComposeModule: NSObject {
  ...
  func start(completion: (() -> Void)?) {
    ...
    let identifierForVendorText = "\(CurrencyCoreStrings.MailComposeModuleLocalization.Identifier.title): \(vendorID)"
    let systemVersionText = "\(CurrencyCoreStrings.MailComposeModuleLocalization.SystemVersion.title): \(systemName) \(systemVersion)"
    let appVersionText = "\(CurrencyCoreStrings.MailComposeModuleLocalization.AppVersion.title): \(appVersion)"
    let messageBody = """
...
"""
    ...
    mailComposeViewController.setSubject(CurrencyCoreStrings.MailComposeModuleLocalization.Subject.title)
