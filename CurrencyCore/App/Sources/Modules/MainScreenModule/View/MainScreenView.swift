  ...
  func createEmptyState() -> some View {
    VStack {
      ScrollView(showsIndicators: false) {
        LottieView(animation: .asset(CurrencyCoreAsset.currencyAnimation.name))
          .resizable()
          .looping()
          .aspectRatio(contentMode: .fit)
        
        Text(CurrencyCoreStrings.MainScreenLocalization.Error.Load.Data.title)
          .font(.fancy.text.largeTitle)
          .foregroundColor(SKStyleAsset.ghost.swiftUIColor)
          .multilineTextAlignment(.center)
        Text(CurrencyCoreStrings.MainScreenLocalization.Error.Load.Data.descriptor)
          .font(.fancy.text.largeTitle)
      ...
      MainButtonView(
        text: CurrencyCoreStrings.MainScreenLocalization
          .Button.Update.title,
        action: {
